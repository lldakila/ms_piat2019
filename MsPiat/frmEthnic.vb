...
Public Class frmEthnic
    ...
    'update score miss
    Sub updateScoreMs()
        da = New OleDbDataAdapter("SELECT * FROM tblEthnicAttire", ultracon)
        dset = New DataSet
        da.Fill(dset, "tblEthnicAttire")

        txtCandi1.Text = dset.Tables("tblEthnicAttire").Rows(0).Item(frmFlash.lblJudge.Text)
        txtCandi2.Text = dset.Tables("tblEthnicAttire").Rows(1).Item(frmFlash.lblJudge.Text)
        txtCandi3.Text = dset.Tables("tblEthnicAttire").Rows(2).Item(frmFlash.lblJudge.Text)
        txtCandi4.Text = dset.Tables("tblEthnicAttire").Rows(3).Item(frmFlash.lblJudge.Text)
        txtCandi5.Text = dset.Tables("tblEthnicAttire").Rows(4).Item(frmFlash.lblJudge.Text)
        txtCandi6.Text = dset.Tables("tblEthnicAttire").Rows(5).Item(frmFlash.lblJudge.Text)
        txtCandi7.Text = dset.Tables("tblEthnicAttire").Rows(6).Item(frmFlash.lblJudge.Text)
        txtCandi8.Text = dset.Tables("tblEthnicAttire").Rows(7).Item(frmFlash.lblJudge.Text)
        txtCandi9.Text = dset.Tables("tblEthnicAttire").Rows(8).Item(frmFlash.lblJudge.Text)
        txtCandi10.Text = dset.Tables("tblEthnicAttire").Rows(9).Item(frmFlash.lblJudge.Text)
    End Sub

    Private Sub frmEthnic_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ramcon()
        showpic
    End Sub

    Private Sub pbCandi1_Click(sender As Object, e As EventArgs) Handles pbCandi1.Click
        frmKeypad.lblCat.Text = "Ethnic Attire"
        frmKeypad.lblTable.Text = "tblEthnicAttire"

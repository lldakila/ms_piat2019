...
Public Class frmQnA
    ...
    Sub updateScoreMs()
        Try
            da = New OleDbDataAdapter("SELECT * FROM tblQnA", ultracon)
            dset = New DataSet
            da.Fill(dset, "tblQnA")

            txtCandi1.Text = dset.Tables("tblQnA").Rows(0).Item(frmFlash.lblJudge.Text)
            txtCandi2.Text = dset.Tables("tblQnA").Rows(1).Item(frmFlash.lblJudge.Text)
            txtCandi3.Text = dset.Tables("tblQnA").Rows(2).Item(frmFlash.lblJudge.Text)
            txtCandi4.Text = dset.Tables("tblQnA").Rows(3).Item(frmFlash.lblJudge.Text)
            txtCandi5.Text = dset.Tables("tblQnA").Rows(4).Item(frmFlash.lblJudge.Text)
            txtCandi6.Text = dset.Tables("tblQnA").Rows(5).Item(frmFlash.lblJudge.Text)
            txtCandi7.Text = dset.Tables("tblQnA").Rows(6).Item(frmFlash.lblJudge.Text)
            txtCandi8.Text = dset.Tables("tblQnA").Rows(7).Item(frmFlash.lblJudge.Text)
            txtCandi9.Text = dset.Tables("tblQnA").Rows(8).Item(frmFlash.lblJudge.Text)
            txtCandi10.Text = dset.Tables("tblQnA").Rows(9).Item(frmFlash.lblJudge.Text)
        Catch ex As Exception
            MsgBox(ex.ToString)
        End Try
    End Sub

    Private Sub frmQnA_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ramcon()
        showpic()
    End Sub

    Private Sub pbCandi1_Click(sender As Object, e As EventArgs) Handles pbCandi1.Click
        frmKeypad.lblCat.Text = "QnA"
        frmKeypad.lblTable.Text = "tblQnA"
